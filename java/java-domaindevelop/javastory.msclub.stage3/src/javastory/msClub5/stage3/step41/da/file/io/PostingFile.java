public class PostingFile {
	...
	public void write(Posting posting) {
		...
		try{
			...
			fileWriter.write(converToStr(posting));
			fileWriter.write("\r\n");
			fileWriter.close();
		} catch (IOException e) {
			e.printStackTrace();
	...
	public Posting read(String postingId) {
		...
		try {
			reader = postingFile.reqeustReader();
			String line = null;

			while((line = reader.readLine()) != null) {

				if (postingFile.hasValueOf("id", postingId, line)){
					posting = converToPosting(line);
					break;
				}
			}
			reader.close();
	...
	public List<Posting> readByTitle(String title) {
		...
		try {
			...
			while ((line = reader.readLinst()) != null) {
