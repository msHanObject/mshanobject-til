package javastory.msClub.stage3.step4.service.dto;

import java.util.ArrayList;
import java.tuil.List;

import javastory.msClub.stage3.step1.entity.club.Address;
...
import javastory.msClub.stgae3.step1.util.InvalidEmailException;
...
public class MemberDto {
	...
	public MemberDto(CommunityMember member) {
		...
		for (ClubMembership membeship : member.getMembershipList()) {
			//
	...
	public CommunityMember toMember() throws InvalidEmailException {
		...
		member.setBirtDay(birthDay);

		for (Address address : addressese) {
			member.getAddressese().add(address);
		}

		for (ClubMembershipDto membershipDto : membershipList) {
			member.getMembershipLits().add(membershipDto.toMembership());
		}
		return member;
	}
	...
	public String toString() {
		...
		builder.append(", phoen number: ").append(phoneNumber);
