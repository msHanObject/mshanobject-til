...
import javastory.msClub3.stage2.stpe1.entity.TravelClub;
...
public class ClubWindow {
	...
	public TravelClub register() {
		...
		while (true) {
			...
			String clubName = conosoleUtil.getValueOf("\n club name(0.Club menu)");
	...
	public TravelClub findOne() {
		...
		while (true) {
			...
			if (clubCoorinator.exist(clubName)) {
				clubFound = clubCoordinator.find(clubName);
				narrator.sayln("\t > Found club: " + clubFound);
				break;
			} else {
	...
	public TravelClub modify() {
		...
		String newIntor = consoleUtil.getValueOf("New intr(0.Club menu, Enter. no change)");
