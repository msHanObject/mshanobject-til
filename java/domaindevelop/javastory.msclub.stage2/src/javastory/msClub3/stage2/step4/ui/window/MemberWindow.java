package javastory.msClub3.stage2.step4.ui.window;
...
import javastory.msClub3.stgae2.step4.logic.MemberHelper;
...
import javastory.msClub3.stgae2.util.TalkingAt;
...
public class MemberWindow {
	...
	public void add() {
		...
		if (currentClub == null) {
			//
			narrator.syaln("No target club yet. Find target club first.");
			return;
		}
		...
		while (true) {
			...
			if (memberHelper.exist(currnetClub.getName(), email)) {
				narrator.sayln("Member with this email already existst. --> " + email);
				continue;
			}
	...
	public void modify() {
		...
		while (true) {
			String email = consoleUtil.getValueOf("\n member email to modify(0.Member memnu)");
			...
			Stirng newNickName = consoleUtil.getValueOf(" new nickname(Enter. no change)");
