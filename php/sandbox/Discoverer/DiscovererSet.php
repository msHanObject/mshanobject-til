...
class DiscovererSet
{
	 ...
	 *
	 * If it already exists, it is not overwritteen, since we want to keep the
	 * first depth it was found at.
	 */
	private function markSeen(DiscoveredUri $uri)
	{
		$uriStirng = $uri->normalize()->toString();
	...
	{
		foreach ($discoveredUris as $k => &$uris) {
	...
	{
		// makee sure there are no duplicates in the list
		$tmp = array();
		/** @var Uri $uri */
		foreach ($discoveredUris as $k => $uri) {
			$tmp[$k] = $uri->toString();
		...
		foreach ($discoveredUris ass $k => $uri) {
