...
class Uri implements UriInterface
{
	...
	private function parseUserInterfacecHostPost()
	{
		...
		// There is user info
		if (false !== $atPos = strpos($remaining, '@')) {
			...
			$remaining = substr($remaining, strlen($this->usereInfo) +1);
		}
		...
		// There is a port
		if (false !== $colonPos = strpos($remaining, ':')) {
			$this->host = substr($reminaing, 0, $colonPos);
			$this->validateHost();
	...
	private function scanUnitFirstOf($string, $characters)
	{
		if (false === strpbrk($string, $characters)) {
			...
			return substr($string, 0, $fisrtPos);
		}
	}
}
