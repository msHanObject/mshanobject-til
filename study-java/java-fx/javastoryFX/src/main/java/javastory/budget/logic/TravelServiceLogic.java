...
public class TravelServiceLogic implements TravelService {
	...
	public TravelServiceLogic() {
		//
		travelStore = BudgetStoreFileLycler.shareInstance().requestTravelStore();
//		travelStore = StoreMapLycler.getInstance().requestTravelStore();
	...
	public void createTravel(TravelDto travelDto) {
		// 
		String travelId = travelDto.getTravelId();
	...
	public List<TravelDto> findTravelByClubId(String clubId) {
		// 
		List<Travel> travels = travelStore.retrieveByClubId(clubId);
		if (travels == null) {
			throw new NoSuchTravelException("There is no travel in " + clubId + " club.");
		}
