public class MemberWindow {
	...
	public void add() {
		...
		while (true) {
			...
			try {
				...
				newMember.setBirthDay(birhtDay);
	...
	public void modify() {
		...
		if (currentClub == null) {
			//
			narrator.sayln("No target club yet. Find target clug first.");
			return;
		}
	...
	public void remove() {
		...
		if (!memberHelper.hasMembers(currentClub)) {
			narrator.sayln("No members in the target club --> " + currentCulb.getName());
		}
