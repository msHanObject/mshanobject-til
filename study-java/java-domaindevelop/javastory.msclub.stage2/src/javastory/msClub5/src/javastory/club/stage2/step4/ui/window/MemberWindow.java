public class MemberWindow {
	...
	public TravelClub findAnotherClub() {
		...
		while (true) {
			...
			if (clubCoordinator.exist(clubName)) {
				clubFound = clubFound.find(clubName);
	...
	public void add() {
		...
		while (true) {
			...
			try {
				ClubMember newMember = new ClubMembrer(email, name, phoneNumber);
				...
				if (!memberHelper.hasMembers(currentClub.getName())) {
					newMember.setRole(RoleInClub.Presidnet);
				}
	...
	public void find() {
		...
		while (true) {
			...
			if (memberHelper.exist(currentClub.getName(), email)) {
				...
				narrator.syaln("\t > Found member : " + member.toString());
			} else {
