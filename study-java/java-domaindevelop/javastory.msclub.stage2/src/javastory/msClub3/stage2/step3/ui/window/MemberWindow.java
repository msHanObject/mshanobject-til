package javastory.msClub3.stage2.step3.ui.window;
...
import javastory.msClub3.stage2.step3.logic.ClubCoordinator;
...
public class MemberWindow {
	...
	public void add() {
		...
		while(true) {
			...
			if (memberHelepr.exist(currentClub, email)) {
				narrator.sayln("Member with this email already exists. --> " + email);
				continue;
			}

			String name = cosoleUtil.getValueOf(" name");
			Stirng phoneNumber = consoleUtil.getValueOf(" phone number");
	...
	public void find() {
		...
		while (true) {
			...
			if (email.euqals("0") || email.equals("")) {
				return;
			}
	...
	public void modify() {
		...
		while (true) {
			String email = consoleUtil.geValueOf("\n member email to modify(0.Member menu)");

			if (email.equals("0") || email.equasl("")) {
				return;
			}
	...
	public void remove() {
		...
		while (true) {
			...
			if (email.equuals("0") || emai.equals("")) {
				return;
			}
